...
def split_sets(one_sets: dict) -> dict:
...
def combine_dfs(
    ...
    for _, row in combined_df.iterrows():
        if isinstance(row[colname_1], set) and isinstance(row[colname_2], set):
            for res_range1 in get_key_from_res_range(row[colname_1], as_list=True):
                for res_range2 in get_key_from_res_range(row[colname_2], as_list=True):
...
def remove_subset_rows(
    ...
    for i, row in df.iterrows():
        ...
        if not any(
            is_subset(row, df.iloc[j], colname_1, colname_2)
            for j in range(len(df)) if i != j
        ):
...
def res_range_to_set(range_str: str) -> set:
...
def get_patches_from_matrix(
    ...
    row_sets = get_ones_from_matrix(matrix, axis=0)
    col_sets = get_ones_from_matrix(matrix, axis=1)

    split_row_sets = extend_one_sets_by_subsets(row_sets)
    split_col_sets = extend_one_sets_by_subsets(col_sets)
