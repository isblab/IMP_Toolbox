...
if __name__ == "__main__":
    ...
    if args.run_type == "single":
        ...
        fitline = sm.nonparametric.lowess(
            tot_score[n_remove:], np.arange(n_remove, n), frac=0.2, return_sorted=False
        )
        exchange_counts_all = sort_the_replica_exchanges_all_temp(
            replica_stack_list, inv_replica_head_dict
        )

        plt.figure(figsize=(10, 10))
        plt.scatter(
            np.arange(n_remove, n), tot_score[n_remove:], color="black", zorder=0, s=10
        )
        plt.scatter(
        ...
        plt.savefig(f"{args.input}/equilibriation_total_score.png")
        plt.close("all")

        plt.figure()
        plt.hist(
            np.where(exchange_indices[n_remove:])[0] + n_remove,
            label="min_temp",
            alpha=0.4,
            color="green",
            bins=100,
        ...
        plt.hist(
            np.where(exchange_counts_all[n_remove:])[0] + n_remove,
            label="all_temp",
            alpha=0.4,
            color="violet",
            bins=100,
        ...
        plt.legend()
        plt.xlim(n_remove, n)
        plt.savefig(f"{args.input}/equilibriation_rex_distribution.png")
        plt.close("all")

        import sys
        # cardiac_desmosome
        if "cardiac_desmosome" == args.sysname:
            big_restraints = [
                "ExcludedVolumeSphere",
                # "ConnectivityRestraint",
                # "GaussianEMRestraint_fullPGDP",
                # "GaussianEMRestraint_PKP_blob",
                "GaussianEMRestraint",
                # "SingleAxisMinGaussianRestraint",
                # "MinimumPairDistanceBindingRestraint",
                # "MembraneBarrierRestraint",
            ]
            resnames = [
                "EVR",
                # "CONN",
        ...
        # # epithelial_desmosome
        # elif "epithelial_desmosome" == args.sysname:
        #     big_restraints = [
        #         "ExcludedVolumeSphere",
        #         "ConnectivityRestraint",
        #         "GaussianEMRestraint_fullPGDP",
        #         "GaussianEMRestraint_PKPStructureGPKP",
        #         "SingleAxisMinGaussianRestraint",
        #         "MinimumPairDistanceBindingRestraint",
        #         "CylinderLocalizationRestraint",
        #     ]
        #     resnames = ["EVR", "Conn", "EMR-PGDP", "EMR-PKP", "SAMGR", "MPDBR", "CLR"]

        # # micos
        # elif "micos" == args.sysname:
        #     big_restraints = [
        #         "ExcludedVolumeSphere",
        #         "ConnectivityRestraint",
        ...
        #     stat_files, _ = get_stat_file_names(args.input)
        #     _head_dict, inv_head_dict = get_stat_head(
        #         args.input, stat_files
        #     )
        #     cl_keys = [
        #         key for key in inv_head_dict.keys()
        #         if "CrossLinkingMassSpectrometryRestraint" in key
        #     ]
        #     big_restraints.append(cl_keys)
        ...
        return_type = "sum"
        sc_sum_arr = np.array(
            [0 for _ in range(n_remove, n)]*len(big_restraints), dtype=np.float32
        ).reshape(len(big_restraints), n - n_remove)
        import umap
        ...
        for reskey, resname in zip(big_restraints, resnames):
            ...
            if eq:
                try:
                    ...
                    print(
                        f"Geyer equilibriation test failed for {reskey}. See the traceback below:"
                    )
                    traceback.print_exc()
                    t_eq = None
                try:
                    ...
                    print(
                        f"Multiscale equilibriation test failed for {reskey}. See the traceback below:"
                    )
                    traceback.print_exc()
                    t_eq_2 = None

            sc_sum_arr[resnames.index(resname), :] = score

        # with open(f"{args.input}/sum_scores_{"_".join(resnames)}.npy", "wb") as f:
        #     np.save(f, sc_sum_arr)
        # exit()
        # embedding = reducer.fit_transform(sc_sum_arr.T)
        ...
        # plt.legend()
        # plt.savefig(f"{args.input}/equilibriation_umap_3d.png")
        # plt.close("all")
        # exit()
        ...
        # plt.savefig(f"{args.input}/equilibriation_umap_2d.png")
        # plt.close("all")
        # exit()
        ...
        # fig.write_html(f"{args.input}/equilibriation_umap_3d_{"_".join(resnames)}.html")
        # exit()
        ...
        # plt.legend()
        # plt.savefig(f"{args.input}/equilibriation_big_restraints_{return_type}.png")
        # plt.close("all")
        ...
        for resname in all_res_info:
            ...
            plt.savefig(f"{args.input}/equilibriation_{resname}.png")
            plt.close("all")

        # temp = np.sum(exchange_indices[n_remove:]) / n_keep * 100
        # print(f"\tTotal min-temp exchanges: {temp:.2f} percent of last {n_keep} frames")
        # print(
        #     f"\tTotal all-temp exchanges: {np.sum(exchange_counts_all[n_remove:]) / n_keep:.2f} per frame"
        # )
        ...
        all_avg_stats_df.to_csv(
            os.path.join(args.input, "avg_stats_all_traj.csv"),
            header=True,
            index=False
        )
